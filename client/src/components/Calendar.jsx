...
const TimeKeepingSchema = yup.object().shape({
  ...
  GioRa: yup
    .string()
    .required("Xin nhập giờ ra")
    .test("is-before-start-time", "Giờ ra phải sau giờ vào", function (value) {
  ...
  const handleUpdate = async (values) => {
    await fetch(`/api/admin/chamcong/update`, {
      ...
      if (resData.error) {
        ...
        setShowModal2(false);
        setChamCongSchema((prev) => {
          const index = prev.findIndex(
            (chamcong) => chamcong.id === resData.newChamCong.id
          );
          if (index !== -1) {
            prev[
              index
            ].title = `${resData.newChamCong.GioVao} - ${resData.newChamCong.GioRa}`;
            prev[index].description = convertDecimalToTime(
              resData.newChamCong.TongGio
            );
          }
          return prev;
        });
        console.log(resData.newChamCong);
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                  <h3 className="text-3xl font-semibold">
                    ...
                    {/* {moment(selectedEvent.start.toString().slice(0,15),'ddd MMM DD YYYY').format('dddd, DD [tháng] MM [năm] YYYY')} */}
                    {chuyenTiengAnhSangViet(
                      selectedEventUpdate.start?.toString()?.slice(0, 15)
                    )}
                  </h3>
                  <button
                    className="p-1 ml-auto bg-transparent border-0 text-black opacity-5 float-right text-3xl leading-none font-semibold outline-none focus:outline-none"
