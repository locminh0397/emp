...
const BonusList = () => {
    ...
    const getBonus = async () => {
        await fetch(`/api/admin/khenthuong`, {
            method: 'GET',
            headers: {
    ...
    let dataBonus = [];
    getDataBonus.getKhenThuong.map((item, index) => {
        dataBonus = [
            ...dataBonus,
            {
                id: item.id,
                stt: index,
                positionCode: item.MaKT,
                title: item.TieuDe,
                thang: item.Thang,
    ...
    const filteredData = dataBonus.filter((row) => {
        return (
    ...
    const handleEdit = (id) => {
        ...
        navigate(`/admin/bonus/${id}/update`);
    };

    const handleDelete = async (id) => {
        if (window.confirm("Bạn chắc chắn muốn xóa khen thưởng này?")) {
            await fetch(`/api/admin/khenthuong/${id}/delete`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`
                }
    ...
    const handleSend = async(id) => {
        await fetch(`/api/admin/khenthuong/${id}/send`,{
            method:'POST',
            headers: {
                Authorization: `Bearer ${token}`
            }
