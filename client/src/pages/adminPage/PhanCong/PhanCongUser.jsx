...
function increaseDay(day, index) {
  ...
  let newDateString = `${date.getDate()}/${
    date.getMonth() + 1
  }/${date.getFullYear()}`;
...
const getCurrentWeek = () => {
  ...
  const currentWeek = Math.ceil(
    ((now - startOfWeek) / 86400000 + startOfWeek.getDay() + 1) / 7
  );
  ...
  const fetchData = async () => {
    try {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        promises.push(
          fetch(
            `/api/admin/phancong/${id}/${formatday(ngayphancong)}/${formatday(
              startDate
            )}`,
            {
              method: "GET",
  ...
  return (
    <div className="bg-white border shadow-sm m-2 z mt-24 p-2 md:p-10 rounded-3xl">
      <button
        ...
        onClick={() => navigate(`/admin/phancong`)}
      >
        Trở lại
      </button>
                        ...
                        >
                          {moment()
                            ...
                            moment()
                              ...
                              .format("dddd")
                              ?.slice(1)}
                          <div>{increaseDay(startDate, index)}</div>
                          <div>
