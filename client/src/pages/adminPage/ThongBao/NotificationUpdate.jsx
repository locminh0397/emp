...
const NotificationUpdate = () => {
  ...
  const getNotification = async (id) => {
    await fetch(`/api/admin/thongbao/${id}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  let dataEmployeeNotification = [];
  for (let i = 0; i < getDataNotification.getThongBao.nhanvien?.length; i++) {
    dataEmployeeNotification.push({
      value: getDataNotification.getThongBao.nhanvien[i].id,
      label: getDataNotification.getThongBao.nhanvien[i].HoTen,
    });
  }
  let dataEmployee = [];
  ...
  const initialValues = {
    title: getDataNotification.getThongBao.TieuDe,
    content: getDataNotification.getThongBao.NoiDung,
    idNhanVien: dataEmployeeNotification,
  };
  const handleReturn = () => {
    navigate("/admin/notification");
  };
  const handleSubmit = async (values) => {
    await fetch(`/api/admin/thongbao/${id}/update`, {
      method: "PUT",
      headers: {
